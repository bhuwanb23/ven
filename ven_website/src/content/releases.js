...
export const RELEASES = [
  {
    ...
    tag: 'minor',
    summary:
      'GUI polish and PHP parity: custom title bar with drag/minimize/close, 150 ms fade transitions between wizard screens, vertically-centered hero screens, and a smoother floating scrollbar. PHP runtime now full-featured with `ven init`/`add`/`remove`/`upgrade`/`sync` support, SHA256 verification, and Composer `--no-scripts`.',
    sections: {
      new: [
        'Custom title bar with window drag, minimize, and close buttons — eliminates OS chrome from the setup wizard.',
        'Fade transitions (150 ms dip-animation) between all eight wizard screens for a polished navigation feel.',
        'Welcome and Done screens vertically centered in the viewport with dynamic 15% top padding.',
        '4 px floating scrollbar — dormant and invisible until hovered, then smoothly fades in.',
        'Window auto-centered on screen at launch via `ViewportCommand::center_on_screen`.',
        'Polished nav footer with a shadow separator line between content and the back/next buttons.',
        '`ven init` support for PHP projects — writes `composer.json` with `require` field.',
        '`ven add` / `ven remove` / `ven upgrade` for PHP projects — routes to `composer require`/`composer remove`/`composer update`.',
        '`ven sync` supports PHP — installs packages from `composer.json`.',
        'Composer `--no-scripts` flag to prevent malicious package scripts during package operations.',
        'SHA256 verification for PHP archive downloads and Python get-pip.py bootstrap script.',
      ],
      improved: [
        'Path traversal protection added to PHP, Go, and Java Unix tar extraction.',
        'PHP version listing now shows all versions from the archives directory.',
        'Shell profile backed up before modification during setup operations.',
        'Font fix: replaced CFF OTF with TrueType TTF for ven-setup GUI, resolving `ttf-parser` incompatibility.',
      ],
      fixed: [
        'Compilation errors in `php_install.rs` resolved.',
        'Cargo fmt formatting fixes across the codebase.',
      ],
    },
  },
  {
