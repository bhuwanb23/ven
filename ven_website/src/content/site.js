...
export const INSTALL = {
  windows: {
    ...
    note:
      'PowerShell 5.1 or 7+. Or download ven-setup-0.2.7-windows-x64.exe above. Installs to %USERPROFILE%\\.ven\\bin. Then ven shell install (once); upgrade with ven update.',
  },
  macos: {
    label: 'macOS',
    cmd: `curl -fsSL ${INSTALL_SH_URL} | sh`,
    note:
      'Or download ven-setup-0.2.7-macos-* above. Installs to ~/.ven/bin and adds PATH. Then ven shell install (once); upgrade with ven update.',
  },
  linux: {
    label: 'Linux',
    cmd: `curl -fsSL ${INSTALL_SH_URL} | sh`,
    note:
      'Or download ven-setup-0.2.7-linux-* above (chmod +x, then run). Bash / Zsh / Fish. Then ven shell install (once); upgrade with ven update.',
  },
  source: {
    label: 'From source',
