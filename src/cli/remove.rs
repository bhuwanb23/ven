...
pub fn cmd_remove(
    ...
    if packages.is_empty() {
        if json {
            ...
            println!("  {} Usage: ven remove <package> [package...] [FLAGS]", "[TIP]".cyan());
            println!();
        }
        return Ok(());
    }
...
fn execute_batch_removal(packages: &[String], force: bool) -> Result<()> {
    ...
    println!("    {} {}", "Removed:".dimmed(), success_count.to_string().green());
    
    if fail_count > 0 {
        println!("    {} {}", "Failed:".dimmed(), fail_count.to_string().red());
    }
    if skipped_count > 0 {
        println!("    {} {}", "Skipped:".dimmed(), skipped_count.to_string().yellow());
    }
...
fn display_dry_run(packages: &[String], force: bool) -> Result<()> {
    println!("\n  {} {}", "ven remove".bold().cyan(), "[DRY RUN]".yellow());
    ...
    for package in packages {
        ...
        let dependents = if force { vec![] } else { find_dependents(package)? };
        total_dependents += dependents.len();
        
        let installed_version = get_installed_version(package).unwrap_or_else(|_| "unknown".to_string());
    ...
    println!();
    println!("  {}", "Summary".bold().cyan());
    println!("    {} {} package(s) would be removed", "Remove:".dimmed(), would_remove.len().to_string().green());
    println!("    {} {} package(s) would be skipped", "Skip:".dimmed(), would_skip.len().to_string().yellow());
    ...
    if force {
        println!();
        println!("  {} Using --force: dependency checks bypassed", "[!]".yellow().bold());
    }
...
fn display_verbose_removal(packages: &[String], force: bool, dry_run: bool) -> Result<()> {
    println!("\n  {} {}", "ven remove".bold().cyan(), "[VERBOSE]".yellow());
    ...
    for package in packages {
        ...
        println!("  {} Size: {}", "Disk Usage:".dimmed(), format_bytes(pkg_size));
        println!("  {} {} direct dependent(s)", "Dependents:".dimmed(), dependents.len());
        println!("  {} {} transitive dependency(ies)", "Dependencies:".dimmed(), transitive_deps.len());
        
        if !dependents.is_empty() {
            println!("    {}", "Direct dependents:".dimmed());
            for (dep, ver) in &dependents {
                println!("      {} {} {}", "•".dimmed(), dep.bold(), format!("({})", ver).dimmed());
            }
        }
        ...
        if !transitive_deps.is_empty() {
            ...
            if transitive_deps.len() > 5 {
                println!("      {} ... and {} more", "...".dimmed(), transitive_deps.len() - 5);
            }
        }
    ...
    println!("    {} {} total disk space", "Disk Space:".dimmed(), format_bytes(total_disk_space));
...
fn cmd_cleanup(json: bool, verbose: bool, dry_run: bool, force: bool) -> Result<()> {
    println!("\n  {} {}", "ven remove".bold().cyan(), "[CLEANUP]".yellow());
...
fn display_cleanup_results(
    ...
    println!();
    println!("  {} Total orphaned disk space: {}", "Total:".dimmed(), format_bytes(total_size));
