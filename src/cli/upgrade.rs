...
pub fn cmd_upgrade(
    ...
    let target_packages = if all {
        ...
        config.packages.keys().cloned().collect::<Vec<String>>()
    } else if packages.is_empty() {
        if json {
            println!("{{\"error\": \"No packages specified\"}}");
        } else {
            println!("\n  {} No packages specified", "[ERROR]".red());
            println!("  {} Usage: ven upgrade <package> [package...] [FLAGS]", "[TIP]".cyan());
...
fn execute_upgrade(
    ...
    println!("    {} {}", "Up to date:".dimmed(), up_to_date_count.to_string().green());
    
    if apply {
        println!("    {} {}", "Upgraded:".dimmed(), upgraded_count.to_string().cyan());
    } else {
        println!("    {} {}", "Available upgrades:".dimmed(), (packages.len() - up_to_date_count).to_string().yellow());
    }
    
    if failed_count > 0 {
        println!("    {} {}", "Failed:".dimmed(), failed_count.to_string().red());
        println!();
    ...
    if !apply && (packages.len() - up_to_date_count) > 0 {
        println!();
        println!("  {} Run  {}  to apply upgrades", "[TIP]".cyan(), "ven upgrade --all --apply".bold());
    }
...
fn process_single_upgrade(
    ...
    if apply {
        if !force && is_major_upgrade(&current_ver, &latest) {
            print!("\n  {} Major upgrade detected. Continue? [y/N]: ", "[!]".yellow().bold());
...
fn display_dry_run_upgrade(packages: &[String], apply: bool) -> Result<()> {
    println!("\n  {} {}", "ven upgrade".bold().cyan(), "[DRY RUN]".yellow());
    ...
    println!("    {} {} can be upgraded", "Upgrade:".dimmed(), can_upgrade.len().to_string().yellow());
    println!("    {} {} already up to date", "Current:".dimmed(), up_to_date.len().to_string().green());
    
    if !failed.is_empty() {
        println!("    {} {} had errors", "Failed:".dimmed(), failed.len().to_string().red());
    }
    
    if !can_upgrade.is_empty() && !apply {
        println!();
        println!("  {} Run  {}  to apply", "[TIP]".cyan(), "ven upgrade --apply".bold());
    }
...
fn display_verbose_upgrade(
    ...
    println!("\n  {} {}", "ven upgrade".bold().cyan(), "[VERBOSE]".yellow());
    println!("  {} Analyzing {} package(s)\n", "[INFO]".cyan(), packages.len());
        ...
        match fetch_npm_info(package) {
            Ok(info) => {
                if let Some(latest) = find_compatible_version(&info, &node_version) {
                    ...
                    println!("  {} Current version: {}", "Current:".dimmed(), 
                        if current_ver == "unknown" { "not installed".to_string() } 
                        else { current_ver.clone() });
                    println!("  {} Latest compatible: {}", "Latest:".dimmed(), latest);
                    println!("  {} Upgrade type: {}", "Type:".dimmed(), upgrade_type.colored());
                    println!("  {} Node {} {}", "Compatibility:".dimmed(), node_version, "✓".green());
                    
                    // Calculate disk space if installed
                    if current_ver != "unknown" {
                        let current_size = calculate_package_size(package).unwrap_or(0);
                        println!("  {} Current size: {}", "Disk Usage:".dimmed(), format_bytes(current_size));
                    }
                    
                    // Show changelog hint
                    if current_ver != latest && current_ver != "unknown" {
                        let changelog_url = format!("https://npmjs.com/package/{}/v/{}", package, latest);
    ...
    println!("    {} {} package(s) analyzed", "Analyzed:".dimmed(), packages.len().to_string().cyan());
    
    let can_upgrade = upgrade_details.iter().filter(|(_, c, l, _, _)| c != l && c != "error" && c != "unknown").count();
    println!("    {} {} package(s) can be upgraded", "Available:".dimmed(), can_upgrade.to_string().yellow());
            ...
            Ok(info) => {
                if let Some(latest) = find_compatible_version(&info, &node_version) {
                    ...
                    if apply && current_ver != latest && !dry_run {
                        ...
                        pkg_info["status"] = json!(if current_ver == latest { "up_to_date" } else { "available" });
...
fn is_major_upgrade(current: &str, latest: &str) -> bool {
    ...
    let current_major = current.split('.').next()
        .and_then(|s| s.parse::<u32>().ok())
        .unwrap_or(0);
    let latest_major = latest.split('.').next()
        .and_then(|s| s.parse::<u32>().ok())
        .unwrap_or(0);
