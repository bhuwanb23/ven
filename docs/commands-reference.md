...
| `ven set global [LANG] [VER]` | Make an **installed** runtime globally available on the **User PATH** — no admin rights, nothing downloaded. `--unset` removes it; no args lists current globals. See [→ set.md](cmds/set.md). |
